...
J I H(I _,I O){r(O?H(X(X(h,U,_),_,O),X(_,h,O)*d):_);}/*multiply*/
J I A(I _){r(Z<<H(_,U));}
J I M(I _){r(_<x?H(u<<_,U):U);}
J/*big oink*/I y(I O,i o){F(fputc(O,o)<h)r(U);r(fflush(o));}
J I S(i ingest,i excrete,I*m/*not feeling well*/,I D,I(*E[u])(I)){
...
int main(int q,Y**v){
	...
	F(fscanf(O,(Y[]){l u-u,-u+h[f],(Y)H(f[h],(Y)H(~H(w,u),u)),(Y)H(w*K,k),h},&s)<=h)G I;r[L(_++)]=X(s,h,M(j));G o;I:
	F(fclose(O)<h)r(k);
	F(H(o,H(~q,u)))G O;
