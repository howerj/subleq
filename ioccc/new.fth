...
defined eforth [if]
  :m #dec dup 0< if [char] - emit then abs u. ;m ( n16 -- )
[else]
...
:a rshift
  ...
  repeat
  x tos MOV ;a
\ Faster negative check? The problem with checking if a number
\ is negative is that "it might be zero" is mixed in with the
\ result, we could check if it is zero first and then ensure
\ it is never zero by adding in one, this would work when 
\ checking the top bit of a number
\
:a opOr
  bwidth w MOV
  x ZERO
...
\ :m mux opMux ;m ( -- : compile opAnd into the dictionary )
...
: > swap op< ; ( n1 n2 -- f : signed greater than )
...
: 1+ #1 + ; ( n -- n : increment a number by one )
: 1- #1 - ; ( n -- n : decrement a number by one )
: 2/ #1 rshift ; ( u -- u : div by two, equivalent to 1 rshift )
...
: u.     #0 <# #s #> space type ; ( u -- )
: . space dup 0< if [char] - emit then abs u. ;
...
: evaluate ( a u -- : evaluate a string )
  ...
  #0 #-1 t' ) lit set-input ( set new input )
